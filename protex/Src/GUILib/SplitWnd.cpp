...
//-----------------------------------------------------------------
//
//					NSplitWnd class implementation
//
//-----------------------------------------------------------------
//-----------------------------------------------------------------
...
bool NSplitWnd::Create(char* name, NRect& rect, NWnd* parent)
{
	//Call Base class
	NWNDCREATE		wc;
	wc.Id			= 1;
	...
	m_hfontNormal = ::CreateFont(
		12, 0, 0, 0, FW_NORMAL, FALSE, FALSE,
...
LRESULT NSplitWnd::WndProc( UINT msg, WPARAM wparam, LPARAM lparam)
{
...
void NSplitWnd::OnPaint()
{
	...
	NGraphics gfx(this);
	gfx.SetBkMode(TRANSPARENT);
	gfx.SetFont(m_hfontNormal);
	gfx.SetTextColor(RGB(255,255,255));
	gfx.FillSolidRect(rc, RGB(90,90,90));
	...
	for (i=mPaneList.Count()-1; i>=0; i--)
	{
		...
		//gfx.GradientVRect(rect, RGB(160,153,147), RGB(131,126,120) );
...
//-----------------------------------------------------------------
udword	NSplitWnd::SplitColumn(udword paneid, sdword dwPosInPercent)
{
...
//!	\return return the new right pane identifier
//-----------------------------------------------------------------
udword	NSplitWnd::SplitRow	(udword paneid, sdword dwPosInPercent)
{
	//Get the pane identifier from the user position
...
//-----------------------------------------------------------------
bool	NSplitWnd::DestroyPane(udword id, udword sidetomerge)
{
	//Check Id
...
//-----------------------------------------------------------------
NSPLITBOX*	NSplitWnd::CreateSplitterBox(NPANEINFO* pane, udword side)
{
	//Array for Hit panes
	NArray<NPANEINFO*> hitlist;
...
// Desc:	Create the Splitters Boxes
//-----------------------------------------------------------------
void	NSplitWnd::DestroySplitterBox(NSPLITBOX* splitbox)
{
...
//-----------------------------------------------------------------
void	NSplitWnd::GetHitPanes(NPANEINFO* pane, udword side, NArray<NPANEINFO*> &array)
{
	if (pane==null)															return;
...
// Desc:
//-----------------------------------------------------------------
udword	NSplitWnd::UpdateCursor(NPoint& mpos)
{
...
//-----------------------------------------------------------------
NPANEINFO*	NSplitWnd::GetPaneInfo(NPoint& pos)
{
...
//-----------------------------------------------------------------
NPANEINFO*	NSplitWnd::GetPaneInfo(udword id)
{
...
//-----------------------------------------------------------------
bool	NSplitWnd::SetPaneWnd(char* _pszCaption, udword id, NWnd* wnd)
{
	//Research the matching pane identifier
//	uword index;
//	NPANEINFO* paneinfo = (NPANEINFO*)mPaneList.GetFirst(index);
...
// Desc:
//-----------------------------------------------------------------
bool	NSplitWnd::MoveSplit(NSPLITBOX* splitbox, NPoint &deltas)
{
...
//	if (_struct->mFlags&MFLG_POS_CHANGED ) {
		...
		if (mSplitBox) {
			...
			::RedrawWindow(m_W32HWnd, null, null, RDW_INTERNALPAINT |  RDW_UPDATENOW);
		}//if (drag)
