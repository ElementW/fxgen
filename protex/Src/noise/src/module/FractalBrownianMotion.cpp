...
Fbm::Fbm ():
  ...
  m_octaveCount  (DEFAULT_FBM_OCTAVE_COUNT),
  m_frequency    (DEFAULT_FBM_FREQUENCY   ),
  m_lacunarity   (DEFAULT_FBM_LACUNARITY  ),
  m_noiseQuality (DEFAULT_FBM_QUALITY     ),
  m_seed         (DEFAULT_FBM_SEED)
{
	offset = 1.0;
	gain = 2.0;
	exponent = 1.0;
...
void Fbm::CalcSpectralWeights ()
{
  ...
  // future version of libnoise.
	...
	for (int i = 0; i < FBM_MAX_OCTAVE; i++) {
...
double Fbm::GetValue (double x, double y, double z) const
{
	...
	for (int curOctave = 0; curOctave < m_octaveCount; curOctave++) {
		...
		value += GradientCoherentNoise3D (x, y, z, seed, m_noiseQuality) * m_pSpectralWeights[curOctave];
		x *= m_lacunarity;
		y *= m_lacunarity;
		z *= m_lacunarity;
  }
