...
double TurbulenceGenerator::GetValue (double x, double y, double z) const
{
	...
	for (int curOctave = 0; curOctave < m_octaveCount; curOctave++) {
		...
		temp *= GradientCoherentNoise3D (nx, ny, nz, seed, m_noiseQuality);
		temp *= m_pSpectralWeights[curOctave];
	...
	if( remainder )
	{
		int seed = (m_seed + m_octaveCount-1) & 0x7fffffff;

		temp = remainder;
		temp *= GradientCoherentNoise3D (nx, ny, nz, seed, m_noiseQuality);
		temp *= m_pSpectralWeights[m_octaveCount-1];
