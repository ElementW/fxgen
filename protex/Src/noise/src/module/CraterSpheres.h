...
namespace noise
{
  ...
  namespace module
  {
    ...
    const double DEFAULT_CRATER_DISPLACEMENT = 1.0;

    /// Default frequency of the seed points for the noise::module::Voronoi
    /// noise module.
    const double DEFAULT_CRATER_FREQUENCY = 1.0;

    /// Default seed of the noise function for the noise::module::Voronoi
    /// noise module.
    const int DEFAULT_CRATER_SEED = 0;

	const int MAX_NUM_CRATERS = 9000;

	class Crater
	{
	public:
		Crater(double rmin, double rmax, double depthmin, double depthmax, int seed);
    ...
    class CraterSpheres: public Module
    {
      ...
      public:
		...
		void SetRadiusMinMax(double rmin, double rmax)
		{
			m_rmin = rmin;
			m_rmax = rmax;
		}

		/*const double Crater_profile(double nsq_rad, double depth);

		const double Dissolve(double nsq_rad);*/

		void SetDepthMinMax(double depthmin, double depthmax)
		{
			m_depthmin = depthmin;
			m_depthmax = depthmax;
		}

		void SetLimits( double lim_min, double lim_max )
		{
			LIM_MIN = lim_min;
			LIM_MAX = lim_max;
		}
