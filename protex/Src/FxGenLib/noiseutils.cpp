...
/////////////////////////////////////////////////////////////////////////////
// NoiseMapBuilderDepthPlane class

NoiseMapBuilderDepthPlane::NoiseMapBuilderDepthPlane ():
  m_isSeamlessEnabled (false),
  m_lowerXBound  (0.0),
  m_lowerZBound  (0.0),
  m_upperXBound  (0.0),
...
void NoiseMapBuilderDepthPlane::Build ()
{
  if ( m_upperXBound <= m_lowerXBound
    || m_upperZBound <= m_lowerZBound
    || m_destWidth <= 0
  ...
  for (int z = 0; z < m_destHeight; z++) {
    ...
    for (int x = 0; x < m_destWidth; x++) {
      float finalValue;
      if (!m_isSeamlessEnabled) {
        finalValue = planeModel.GetValue (xCur, m_depth, zCur);
      } else {
        double swValue, seValue, nwValue, neValue;
        swValue = planeModel.GetValue (xCur          , m_depth, zCur          );
        seValue = planeModel.GetValue (xCur + xExtent, m_depth, zCur          );
        nwValue = planeModel.GetValue (xCur          , m_depth, zCur + zExtent);
        neValue = planeModel.GetValue (xCur + xExtent, m_depth, zCur + zExtent);
        double xBlend = 1.0 - ((xCur - m_lowerXBound) / xExtent);
        double zBlend = 1.0 - ((zCur - m_lowerZBound) / zExtent);
        double z0 = LinearInterp (swValue, seValue, xBlend);
        double z1 = LinearInterp (nwValue, neValue, xBlend);
...
/////////////////////////////////////////////////////////////////////////////
// NoiseMapConcentricDepthSphere class

NoiseMapConcentricDepthSphere::NoiseMapConcentricDepthSphere ():
  m_eastLonBound  (0.0),
  m_northLatBound (0.0),
  m_southLatBound (0.0),
  m_westLonBound  (0.0),
...
void NoiseMapConcentricDepthSphere::Build ()
{
  if ( m_eastLonBound <= m_westLonBound
    || m_northLatBound <= m_southLatBound
    || m_destWidth <= 0
  ...
  // Create the plane model.
  model::ConcentricDepthSphere sphereModel;
  sphereModel.SetModule (*m_pSourceModule);
  sphereModel.SetRadius(m_radius);
