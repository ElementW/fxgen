...
void NMenuCtrl::OnPaint()
{
	...
	if (m_cstrText.Length())
	{
		...
		rcText.left+=2;
		dc.DrawText(m_cstrText.Buffer() , rcText, NDT_END_ELLIPSIS|NDT_VCENTER|NDT_SINGLELINE, RGBA(0,0,0,255) );
	...
	for (udword i=0; i<dwCount; i++)
	{
		...
		dc.DrawText(pitem->strName.Buffer(), rcText, NDT_END_ELLIPSIS|NDT_VCENTER|NDT_SINGLELINE, RGBA(0,0,0,255) );
