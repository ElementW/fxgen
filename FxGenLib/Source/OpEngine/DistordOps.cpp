...
udword NRotoZoomOp::Process(float _ftime, SEngineState& _state)
{
	...
	byVal = m_pcvarsBloc->GetUByteValue(0, 0.0f);
	if (byVal!=0)
	{
		w=1<<((udword)(byVal-1));
	...
	byVal = m_pcvarsBloc->GetUByteValue(1, 0.0f);
	if (byVal!=0)
	{
		h=1<<((udword)(byVal-1));
	...
	float fCenterX = m_pcvarsBloc->GetFloatValue(2, _ftime);	//-0.5 <> +0.1
	float fCenterY = m_pcvarsBloc->GetFloatValue(3, _ftime);	//-0.5 <> +0.1
	float fRotate = m_pcvarsBloc->GetFloatValue(4, _ftime);		//0.0 <> 1.0 => 0 => 2xPi
	float fZoomX = m_pcvarsBloc->GetFloatValue(5, _ftime);
	float fZoomY = m_pcvarsBloc->GetFloatValue(6, _ftime);
...
udword NDistortOp::Process(float _ftime, SEngineState& _state)
{
	...
	/////////////////////////////////////////
	//Get Variables Values
	float fPower = (float)m_pcvarsBloc->GetUByteValue(0, _ftime);	//0.0<->8.0
...
udword NVortexOp::Process(float _ftime, SEngineState& _state)
{
	...
	//Get Variables Values
	float fCenterX = m_pcvarsBloc->GetFloatValue(0, _ftime);
	float fCenterY = m_pcvarsBloc->GetFloatValue(1, _ftime);
	float fRayX = m_pcvarsBloc->GetFloatValue(2, _ftime);
	float fRayY = m_pcvarsBloc->GetFloatValue(3, _ftime);
	float fTwist = m_pcvarsBloc->GetFloatValue(4, _ftime);
	...
	float radians = fTwist * 100.0f * nv_to_rad;
