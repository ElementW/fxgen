...
{
	char* pszname = m_pcvarsBloc->GetStringValue(0, 0.0f);
	return pszname;
}
...
{
	return m_pcvarsBloc->GetStringValue(0, 0.0f);
}
...
const char* NLoadOp::GetUserName()
{
	...
	NOperatorFx* popRef = (NOperatorFx*)m_pcvarsBloc->GetObjectValue(0, 0.0f);
...
udword NLoadOp::Process(float _ftime, SEngineState& _state)
{
	...
	//Get Variables Values
	NOperatorFx* popRef = (NOperatorFx*)m_pcvarsBloc->GetObjectValue(0, _ftime);
...
udword NChannelAnimFX1Op::Process(float _ftime, SEngineState& _state)
{
	...
	ubyte byType = m_pcvarsBloc->GetUByteValue(1, _ftime);
	float fFactor = m_pcvarsBloc->GetFloatValue(4, _ftime);
