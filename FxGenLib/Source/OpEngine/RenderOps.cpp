...
udword NFlatOp::Process(float _ftime, SEngineState& _state)
{
	...
	udword w=1<<((udword)m_pcvarsBloc->GetUByteValue(0, 0.0f));
	udword h=1<<((udword)m_pcvarsBloc->GetUByteValue(1, 0.0f));

	w=(udword) ((float)w*_state.fDetailFactor);
	h=(udword) ((float)h*_state.fDetailFactor);

	NRGBA col =	m_pcvarsBloc->GetUDwordValue(2, _ftime);
...
udword NCloudOp::Process(float _ftime, SEngineState& _state)
{
	...
	//Get Variables Values
	NRGBA color0 = m_pcvarsBloc->GetUDwordValue(2, _ftime);
	NRGBA color1 = m_pcvarsBloc->GetUDwordValue(3, _ftime);
	ubyte byAmp = m_pcvarsBloc->GetUByteValue(4, _ftime);
	uword wSeed = m_pcvarsBloc->GetUWordValue(5, _ftime);
...
udword NGradientOp::Process(float _ftime, SEngineState& _state)
{
	...
	//Get Variables Values
	sdword w=1<<((udword)m_pcvarsBloc->GetUByteValue(0, 0.0f));
	sdword h=1<<((udword)m_pcvarsBloc->GetUByteValue(1, 0.0f));

	w=(sdword) ((float)w*_state.fDetailFactor);
	h=(sdword) ((float)h*_state.fDetailFactor);

	NRGBA colA = m_pcvarsBloc->GetUDwordValue(2, _ftime);
	NRGBA colB = m_pcvarsBloc->GetUDwordValue(3, _ftime);
	NRGBA colC = m_pcvarsBloc->GetUDwordValue(4, _ftime);
	NRGBA colD = m_pcvarsBloc->GetUDwordValue(5, _ftime);
...
udword NCellOp::Process(float _ftime, SEngineState& _state)
{
	...
	ubyte byRegularity = m_pcvarsBloc->GetUByteValue(2, _ftime);
	ubyte byDensity		= m_pcvarsBloc->GetUByteValue(3, _ftime);
	NRGBA col					= m_pcvarsBloc->GetUDwordValue(4, _ftime);
	uword wSeed				= m_pcvarsBloc->GetUWordValue(5, _ftime);
	ubyte chessboard	= m_pcvarsBloc->GetUByteValue(6, _ftime);
	ubyte pattern			= m_pcvarsBloc->GetUByteValue(7, _ftime);
...
udword NNoiseOp::Process(float _ftime, SEngineState& _state)
{
	...
	uword wSeed = m_pcvarsBloc->GetUWordValue(3, _ftime);
	SetSeedValue(wSeed);
