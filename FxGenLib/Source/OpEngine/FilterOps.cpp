...
udword NBlurOp::Process(float _ftime, SEngineState& _state)
{
	...
	float fWidth = m_pcvarsBloc->GetFloatValue(0, _ftime);
	float fHeight = m_pcvarsBloc->GetFloatValue(1, _ftime);
	ubyte byAmplify = m_pcvarsBloc->GetUByteValue(2, _ftime);
	ubyte byType = m_pcvarsBloc->GetUByteValue(3, _ftime);
...
udword NColorsOp::Process(float _ftime, SEngineState& _state)
{
	...
	//Get Variables Values
	NRGBA ColorBase			= m_pcvarsBloc->GetUDwordValue(0, _ftime);
	NRGBA ColorPer			= m_pcvarsBloc->GetUDwordValue(1, _ftime);
	ubyte byBrihtness		= m_pcvarsBloc->GetUByteValue(2, _ftime);
	ubyte byContrast		= m_pcvarsBloc->GetUByteValue(3, _ftime);
	ubyte bySaturation	= m_pcvarsBloc->GetUByteValue(4, _ftime);
	ubyte byAlpha				= m_pcvarsBloc->GetUByteValue(5, _ftime);
...
udword NLightOp::Process(float _ftime, SEngineState& _state)
{
	...
	/////////////////////////////////////////
	//Get Variables Values
	NRGBA Ambiant = m_pcvarsBloc->GetUDwordValue(0, _ftime);
	NRGBA Diffuse = m_pcvarsBloc->GetUDwordValue(1, _ftime);
	NRGBA Specular = m_pcvarsBloc->GetUDwordValue(2, _ftime);

	ubyte byPosX = m_pcvarsBloc->GetUByteValue(3, _ftime);
	ubyte byPosY = m_pcvarsBloc->GetUByteValue(4, _ftime);
	ubyte byPosZ = m_pcvarsBloc->GetUByteValue(5, _ftime);		//255-near 0-far

	ubyte bySpecPower = m_pcvarsBloc->GetUByteValue(6, _ftime);
	ubyte byBumpPower = m_pcvarsBloc->GetUByteValue(7, _ftime);
...
udword NNormalsOp::Process(float _ftime, SEngineState& _state)
{
	...
	/////////////////////////////////////////
	//Get Variables Values
	ubyte byAmp = m_pcvarsBloc->GetUByteValue(0, _ftime);
...
udword NAbnormalsOp::Process(float _ftime, SEngineState& _state)
{
	...
	/////////////////////////////////////////
	//Get Variables Values
	float w = m_pcvarsBloc->GetFloatValue(1, _ftime);
	float x = m_pcvarsBloc->GetFloatValue(2, _ftime);
	float y = m_pcvarsBloc->GetFloatValue(3, _ftime);
	float z = m_pcvarsBloc->GetFloatValue(4, _ftime);

	float sensitivity = (float)m_pcvarsBloc->GetUByteValue(6, _ftime) / 255.f;

	ubyte comp = m_pcvarsBloc->GetUByteValue(7, _ftime);
	ubyte mirror = m_pcvarsBloc->GetUByteValue(8, _ftime);
...
{
	VAR(eubyte,	true, "Amplify",	"64", "NUbyteProp",0.0f, 255.0f, 1.0f)	//0
};
...
udword NThresholdOp::Process(float _ftime, SEngineState& _state)
{
	...
	//Get Variables Values
	ubyte byThreshold = m_pcvarsBloc->GetUByteValue(0, _ftime);
	ubyte byRatio = m_pcvarsBloc->GetUByteValue(1, _ftime);
	ubyte byMode = m_pcvarsBloc->GetUByteValue(2, _ftime);
...
udword NDilateOp::Process(float _ftime, SEngineState& _state)
{
	...
	//Get Variables Values
	ubyte byIterations = m_pcvarsBloc->GetUByteValue(0, _ftime);

	NRGBA* pPxInter = (NRGBA*)NMemAlloc(w*h*sizeof(NRGBA));

	for (sdword i=0; i<((sdword)byIterations)+1; ++i)
	{
		udword index = 0;
		NRGBA* pPxSrc;
		NRGBA* pPxDst;
...
udword NAlphaMaskOp::Process(float _ftime, SEngineState& _state)
{
	...
	//Get Variables Values
	ubyte colormask = m_pcvarsBloc->GetUByteValue(0, _ftime);
