		...
		*_l>>m_dwVarsCount;
		for (udword i=0; i<m_dwVarsCount; i++)
		{
			...
			switch (pdesc->eVarType)
			{
				...
				case erefobj:	SetObjectValue(i, 0, _l->GetMappedObj());	break;
...
void NVarsBloc::RemoveVarsRef(NObject* _pobj)
{
	for (udword i=0; i<m_dwVarsCount; i++)
	{
		NVarValue* pval = m_paVarsValues + i;
		NVarsBlocDesc* pdesc = m_pcvarsblocDesc + i;
		if (pdesc->eVarType==erefobj && pval->pcRefObj==_pobj)
			SetObjectValue(i, 0, (NObject*)null);
	}
...
//-----------------------------------------------------------------
ubyte NVarsBloc::GetUByteValue(udword _idx, float _fTime)
{
	if (m_paVarsValues[_idx].pcCtrlObj)
		return (ubyte)((NController*)m_paVarsValues[_idx].pcCtrlObj)->GetValue(_fTime);
	return m_paVarsValues[_idx].byVal;
}

uword NVarsBloc::GetUWordValue(udword _idx, float _fTime)
{
	if (m_paVarsValues[_idx].pcCtrlObj)
		return (uword)((NController*)m_paVarsValues[_idx].pcCtrlObj)->GetValue(_fTime);
	return m_paVarsValues[_idx].wVal;
}

udword NVarsBloc::GetUDwordValue(udword _idx, float _fTime)
{
	if (m_paVarsValues[_idx].pcCtrlObj)
		return (udword)((NController*)m_paVarsValues[_idx].pcCtrlObj)->GetValue(_fTime);
	return m_paVarsValues[_idx].dwVal;
}

float NVarsBloc::GetFloatValue(udword _idx, float _fTime)
{
	if (m_paVarsValues[_idx].pcCtrlObj)
		return ((NController*)m_paVarsValues[_idx].pcCtrlObj)->GetValue(_fTime);
	return m_paVarsValues[_idx].fVal;
}

NObject* NVarsBloc::GetObjectValue(udword _idx, float _fTime)
{
	return m_paVarsValues[_idx].pcRefObj;
}

char* NVarsBloc::GetStringValue(udword _idx, float _fTime)
{
	return m_paVarsValues[_idx].szVal;
}
...
//-----------------------------------------------------------------
void NVarsBloc::SetUByteValue(udword _idx, float _fTime, ubyte _val)
{
	m_paVarsValues[_idx].byVal = _val;
}

void NVarsBloc::SetUWordValue(udword _idx, float _fTime, uword	_val)
{
	m_paVarsValues[_idx].wVal = _val;
}

void NVarsBloc::SetUDwordValue(udword _idx, float _fTime, udword	_val)
{
	m_paVarsValues[_idx].dwVal = _val;
}

void NVarsBloc::SetFloatValue(udword _idx, float _fTime, float _val)
{
	m_paVarsValues[_idx].fVal = _val;
}

void NVarsBloc::SetObjectValue(udword _idx, float _fTime, NObject* _val)
{
...
void NVarsBloc::SetStringValue(udword _idx, float _fTime, const char*	_val)
{
