...
//		op->parent->update(op->op, op->op->m_wPosX, op->op->m_wPosY);
    ...
    if (fcb)
    {
       	if(fcb->get_current_folder().size())
			dirname = fcb->get_current_folder();

//        if (dynamic_cast<NVectorOp*>(op->op))
//        {
...
//        else
//        {
        	if(fcb->get_filename().size())
				strncpy(val->szVal, fcb->get_filename().c_str(), sizeof(val->szVal));
        	return;
//		}
    }
}
