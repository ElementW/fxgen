...
//!	\brief	Window proc
//-----------------------------------------------------------------
LRESULT NTreeNodeCtrl::WndProc( UINT msg, WPARAM wparam, LPARAM lparam)
{
...
void NTreeNodeCtrl::OnPaint()
{
	...
	dc.SetBrush(RGB(64,64,64));
	dc.SetPen(1, RGB(64,64,64));
	...
	for (udword i=0; i<m_carrayItemsSortedPos.Count(); i++)
	{
		...
		//Row Background
		if (m_dwCurItemIdx==idx)	dc.FillSolidRect(item.rcItem,	RGB(160,159,167));
		//else if (i%2)						dc.FillSolidRect(item.rcItem,	RGB(107,107,107));
		//else										dc.FillSolidRect(item.rcItem,	RGB(113,113,113));

		//Display Text
		NRect rcText = item.rcItem;
		rcText.left+=((item.dwDepth+1)*TN_ITEMIDENT);
		dc.DrawText(item.pcobj->GetName(), rcText, DT_END_ELLIPSIS|DT_VCENTER|DT_SINGLELINE);
...
void NTreeNodeCtrl::DisplayTreeNode(NTreeNode* _proot)
{
	...
	m_prootNode = _proot;
	m_carrayItemsSortedPos.Clear();
	m_dwLastUpdate = (udword)GetTickCount();
